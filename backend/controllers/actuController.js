...
exports.getAllActus = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: false,
        message: "Failed to retrieve actus",
        error: error.message,
      });
  }
};
...
exports.getActuById = async (req, res) => {
  try {
    ...
    if (!actu) {
      return res
        .status(404)
        .json({ success: false, message: "Actu not found" });
    }
    ...
    res
      ...
      .json({
        success: false,
        message: "Failed to retrieve actu",
        error: error.message,
      });
  }
};
...
exports.createActu = async (req, res, next) => {
  ...
  try {
    ...
    if (req.file) {
      ...
      actuData.image = `${req.file.filename}`;
    ...
    const formattedNewActu = {
      ...
      creatorName: createdActuWithCreator.creator
        ? createdActuWithCreator.creator.name
        : null,
    ...
    if (req.file) {
      fs.unlink(req.file.path, (err) => {
        if (err)
          console.error("Error deleting uploaded file on creation error:", err);
      });
    }
    next(error);
  }
...
exports.updateActu = async (req, res, next) => {
  ...
  try {
    ...
    if (!actu) {
      if (req.file) {
        fs.unlink(req.file.path, (err) => {
          if (err)
            console.error(
              "Error deleting uploaded file for not found actu:",
              err
            );
        });
      }
    ...
    if (req.file) {
      ...
      if (actu.image) {
        ...
        if (fs.existsSync(oldImagePath)) {
          fs.unlink(oldImagePath, (err) => {
            if (err)
              console.error("Error deleting old actu image on clear:", err);
          });
        }
      }
    ...
    const formattedUpdatedActu = {
      ...
      creatorName: updatedActuWithCreator.creator
        ? updatedActuWithCreator.creator.name
        : null,
...
exports.deleteActu = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Actualité supprimée avec succès" });
  } catch (error) {
