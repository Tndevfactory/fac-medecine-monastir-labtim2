  ...
  if (user) {
    ...
    if (typeof user.universityEducation === "string") {
      try {
        ...
        console.error(
          "Error parsing universityEducation for user:",
          user.id,
          e
        );
...
exports.getAllUsers = async (req, res, next) => {
  try {
    ...
    console.log(
      "Final whereClause for Sequelize:",
...
exports.getUserById = async (req, res, next) => {
  try {
    ...
    if (
      user.isArchived &&
      (!req.user || (req.user.role !== "admin" && req.user.id !== user.id))
    ) {
      return res
        .status(404)
        .json({
          success: false,
          message: "Utilisateur non trouvé ou archivé.",
        });
    }
...
exports.createUser = async (req, res, next) => {
  ...
  try {
    ...
    if (userExists) {
      if (req.file) {
        fs.unlink(req.file.path, (err) => {
          if (err)
            console.error(
              "Error deleting uploaded file on duplicate user:",
              err
            );
        });
      }
    ...
    if (req.file) {
      ...
      imagePath = `${req.file.filename}`; // Store as /uploads/filename.ext
    }

    // Ensure parsing for creation as well
    const parsedExpertises = expertises ? JSON.parse(expertises) : [];
    const parsedResearchInterests = researchInterests
      ? JSON.parse(researchInterests)
      : [];
    const parsedUniversityEducation = universityEducation
      ? JSON.parse(universityEducation)
      : [];
...
exports.updateUser = async (req, res, next) => {
  ...
  try {
    ...
    if (req.user.role !== "admin" && req.user.id !== user.id) {
      if (req.file) {
        fs.unlink(req.file.path, (err) => {
          if (err)
            console.error(
              "Error deleting uploaded file on update error (unauthorized):",
              err
            );
        });
      }
    ...
    if (biography !== undefined)
      updateData.biography = biography === "" ? null : biography;

    // Parse JSON fields for update
    if (expertises !== undefined)
      updateData.expertises = JSON.parse(expertises);
    if (researchInterests !== undefined)
      updateData.researchInterests = JSON.parse(researchInterests);
    if (universityEducation !== undefined)
      updateData.universityEducation = JSON.parse(universityEducation);

    if (expirationDate !== undefined)
      updateData.expirationDate = expirationDate === "" ? null : expirationDate;
    ...
    if (orcid !== undefined && orcid !== user.orcid) {
      ...
      if (existingUserWithOrcid && existingUserWithOrcid.id !== user.id) {
        if (req.file) {
          fs.unlink(req.file.path, (err) => {
            if (err)
              console.error(
                "Error deleting uploaded file on duplicate ORCID:",
                err
              );
          });
        }
      ...
      return res
        ...
        .json({
          success: false,
          message: "Cet ORCID est déjà utilisé par un autre utilisateur.",
        });
      ...
      // orcid is provided and is the same or empty to clear
      updateData.orcid = orcid === "" ? null : orcid;
    }

    // Image handling logic:
    if (req.file) {
      // A new file was uploaded (either original or cropped)
      ...
      updateData.image = `${req.file.filename}`; // Store new image path
    } else if (imageControl === "DELETE_IMAGE_SIGNAL") {
      // Frontend explicitly sent signal to remove image
      if (user.image) {
        const oldImagePath = path.join(__dirname, "..", user.image);
        if (fs.existsSync(oldImagePath)) {
          fs.unlink(oldImagePath, (err) => {
            if (err)
              console.error(
                "Error deleting old profile image on explicit delete signal:",
                err
              );
          });
        }
    ...
    if (user.image) {
      ...
      if (fs.existsSync(imagePath)) {
        fs.unlink(imagePath, (err) => {
          if (err)
            console.error("Error deleting profile image on user delete:", err);
        });
      }
    }
...
exports.sendUserCredentialsEmail = async (req, res, next) => {
  ...
  try {
    ...
    console.log(
      `Generated new temporary password for ${user.email}: ${newTemporaryPassword}`
    );
    ...
    res
      ...
      .json({
        success: true,
        message:
          "Identifiants envoyés par email avec succès. Le mot de passe a été réinitialisé temporairement.",
      });
  } catch (error) {
    ...
    console.log(
      "--- END Backend (UserController - sendUserCredentialsEmail) ---"
    );
  }
};
