...
const UPLOAD_DIR_NAME = "uploads"; // Consistent directory name
...
const getImageFullPath = (imageLocalPathInDb) => {
  ...
  if (!cleanPath.startsWith("uploads/")) {
    console.warn(
      `[getImageFullPath] Path "${cleanPath}" did not start with "uploads/", prepending it.`
    );
  ...
  console.log(
    `[getImageFullPath] Final output URL for frontend: "${finalWebPath}"`
  );
  return finalWebPath;
};
...
exports.getMainPresentation = async (req, res) => {
  ...
  try {
    ...
    if (!presentation) {
      console.log(
        "[getMainPresentation] No presentation found, creating a default one."
      );
      ...
      return res
        .status(200)
        .json({ success: true, data: presentation.toJSON() });
    }

    console.log("[getMainPresentation] Found existing presentation.");
    console.log(
      "[getMainPresentation] Raw contentBlocks from DB (BEFORE model getter processing):",
      presentation.getDataValue("contentBlocks")
    );

    const contentBlocksFromModel = presentation.contentBlocks;
    console.log(
      "[getMainPresentation] ContentBlocks after model getter (parsed array):",
      JSON.stringify(contentBlocksFromModel, null, 2)
    );

    const formattedBlocks = (
      Array.isArray(contentBlocksFromModel) ? contentBlocksFromModel : []
    ).map((block, index) => {
      if (block.type === "image" && block.url) {
        console.log(
          `[getMainPresentation] Processing image block ${index}. Raw URL (from model getter): "${block.url}"`
        );
        if (
          ...
          console.log(
            `[getMainPresentation] Formatted URL for block ${index} (via getImageFullPath): "${formattedUrl}"`
          );
          ...
          console.log(
            `[getMainPresentation] Block ${index} URL is external, keeping as is: "${block.url}"`
          );
        }
      }
      return block;
    ...
    if (
      presentation.directorImage &&
      !presentation.directorImage.startsWith("http://") &&
      !presentation.directorImage.startsWith("https://")
    ) {
      formattedDirectorImage = getImageFullPath(presentation.directorImage);
      console.log(
        `[getMainPresentation] Formatted Director Image URL: "${formattedDirectorImage}"`
      );
    } else {
    ...
    console.log(
      "[getMainPresentation] Final Response data being sent to frontend:",
    ...
    console.log(
      "--- END Backend (PresentationController - getMainPresentation) ---"
    );
  }
};
...
exports.updateMainPresentation = async (req, res, next) => {
  ...
  try {
    ...
    if (!presentation) {
      console.error(
        "[updateMainPresentation] Main presentation content not found, this should not happen."
      );
      return res
        .status(404)
        .json({
          success: false,
          message: "Main presentation content not found.",
        });
    }

    const incomingContentBlocks = req.body.contentBlocks
      ? JSON.parse(req.body.contentBlocks)
      : [];
    console.log(
      "[updateMainPresentation] Parsed incomingContentBlocks (from frontend):",
      JSON.stringify(incomingContentBlocks, null, 2)
    );
    ...
    for (let i = 0; i < incomingContentBlocks.length; i++) {
      ...
      if (block.type === "image") {
        ...
        const uploadedFile =
          req.files && req.files[imageFileKey]
            ? req.files[imageFileKey][0]
            : null;
        ...
        if (uploadedFile) {
          ...
          const oldContentBlocks = Array.isArray(presentation.contentBlocks)
            ? presentation.contentBlocks
            : [];
          ...
          if (
            ...
            oldBlock.type === "image" &&
            oldBlock.url &&
            !oldBlock.url.startsWith("http")
          ) {
            const oldImagePathToDelete = path.join(
              __dirname,
              "..",
              oldBlock.url
            );
            if (fs.existsSync(oldImagePathToDelete)) {
              fs.unlink(oldImagePathToDelete, (err) => {
                if (err)
                  console.error(
                    `Error deleting old image for block ${i}:`,
                    err
                  );
              });
            }
          ...
          const oldContentBlocks = Array.isArray(presentation.contentBlocks)
            ? presentation.contentBlocks
            : [];
          const originalBlock = oldContentBlocks.find(
            (pBlock) => pBlock.id === block.id
          );
          if (
            originalBlock &&
            originalBlock.type === "image" &&
            originalBlock.url &&
            !originalBlock.url.startsWith("http")
          ) {
            const oldImagePathToDelete = path.join(
              __dirname,
              "..",
            ...
            if (fs.existsSync(oldImagePathToDelete)) {
              fs.unlink(oldImagePathToDelete, (err) => {
                if (err)
                  console.error(
                    `Error deleting old image on explicit clear: ${oldImagePathToDelete}`,
                    err
                  );
              });
            }
          ...
          // Existing image without new upload: ensure properties are maintained
          // *****************************************************************
    ...
    const oldImagePathsInDb = (
      Array.isArray(presentation.contentBlocks)
        ? presentation.contentBlocks
        : []
    )
      .filter(
        (block) =>
          block.type === "image" && block.url && !block.url.startsWith("http")
      )
      .map((block) => block.url);
      ...
      if (!pathsToKeepSet.has(oldPath)) {
        ...
        if (fs.existsSync(fullPathToDelete)) {
          fs.unlink(fullPathToDelete, (err) => {
            if (err)
              console.error(
                `Error deleting unreferenced old image: ${fullPathToDelete}`,
                err
              );
          });
        }
    ...
    const fieldsToUpdate = {
      ...
      counter1Value: req.body.counter1Value
        ? parseInt(req.body.counter1Value, 10)
        : 0,
      counter1Label: req.body.counter1Label || "Permanents",
      counter2Value: req.body.counter2Value
        ? parseInt(req.body.counter2Value, 10)
        : 0,
      counter2Label: req.body.counter2Label || "Articles impactés",
      counter3Value: req.body.counter3Value
        ? parseInt(req.body.counter3Value, 10)
        : 0,
      counter3Label: req.body.counter3Label || "Articles publiés",
      contentBlocks: newContentBlocks, // This is the processed ARRAY
    };

    const directorImageFile =
      req.files && req.files["directorImage"]
    ...
    if (directorImageFile) {
      if (
        ...
        const oldDirectorImagePath = path.join(
          __dirname,
          "..",
          presentation.directorImage
        );
        if (fs.existsSync(oldDirectorImagePath)) {
          fs.unlink(oldDirectorImagePath, (err) => {
            if (err)
              console.error(
                `Error deleting old director image: ${oldDirectorImagePath}`,
                err
              );
          });
        }
      ...
      req.body.directorImage === "null" ||
      req.body.directorImage === ""
    ) {
      if (
        presentation.directorImage &&
        !presentation.directorImage.startsWith("http")
        ...
        if (fs.existsSync(oldDirectorImagePath)) {
          fs.unlink(oldDirectorImagePath, (err) => {
            if (err)
              console.error(
                `Error deleting director image on clear: ${oldDirectorImagePath}`,
                err
              );
          });
        }
      ...
      req.body.directorImage &&
      !req.body.directorImage.startsWith("http")
    ) {
    ...
    const contentBlocksForResponse = Array.isArray(presentation.contentBlocks)
      ? presentation.contentBlocks
      : [];
    ...
    console.log(
      "[updateMainPresentation] Response data being sent:",
      JSON.stringify(responseData, null, 2)
    );
    res.status(200).json({ success: true, data: responseData });
  } catch (error) {
        ...
        if (Array.isArray(req.files[key])) {
          req.files[key].forEach((file) => {
            fs.unlink(file.path, (err) => {
              if (err)
                console.error(
                  `Error deleting newly uploaded file on update error: ${file.path}`,
                  err
                );
            });
          });
    ...
    console.log(
      "--- END Backend (PresentationController - updateMainPresentation) ---"
    );
  }
};
