...
exports.createCarouselItem = async (req, res, next) => {
  ...
  try {
    ...
    if (!req.file) {
      // Image is mandatory for a new carousel item
      // This is the custom check to give a more specific error than notNullViolation
      return res
        .status(400)
        .json({
          success: false,
          message: "Une image est requise pour créer un élément de carrousel.",
        });
    }
    ...
    if (!order || isNaN(Number(order))) {
      // Check if order is provided and is a valid number
      if (req.file) {
        // Clean up uploaded file if validation fails here
        fs.unlink(req.file.path, (err) => {
          if (err)
            console.error(
              "Error deleting uploaded file on order validation error:",
              err
            );
        });
      }
    ...
    const existingItemWithOrder = await CarouselItem.findOne({
    ...
    if (existingItemWithOrder) {
      ...
      if (req.file) {
        fs.unlink(req.file.path, (err) => {
          if (err)
            console.error(
              "Error deleting uploaded file on duplicate order error:",
              err
            );
        });
      }
      return res
        ...
        .json({
          success: false,
          message: `Un élément avec l'ordre '${order}' existe déjà. Veuillez choisir un numéro d'ordre différent.`,
        });
    }
    ...
    if (newItemJson.imageUrl) {
      // Only format if image exists
    ...
    if (req.file) {
      fs.unlink(req.file.path, (err) => {
        if (err)
          console.error("Error deleting uploaded file on create error:", err);
      });
    }
    next(error); // Pass error to global error handler
  } finally {
    console.log(
      "--- END Backend (CarouselController - createCarouselItem) ---"
    );
  }
};
...
exports.updateCarouselItem = async (req, res, next) => {
  ...
  try {
    ...
    console.log(
      "DEBUG: Combined validation check (order === undefined || order === null || isNaN(Number(order))):",
    ...
    if (order === undefined || order === null || isNaN(Number(order))) {
      if (req.file) {
        fs.unlink(req.file.path, (err) => {
          if (err)
            console.error(
              "Error deleting uploaded file on update order validation error:",
              err
            );
        });
      }
    ...
    if (Number(order) !== item.order) {
      // Only check uniqueness if order is actually changing
      const existingItemWithNewOrder = await CarouselItem.findOne({
        where: { order: Number(order) },
      });
      if (existingItemWithNewOrder && existingItemWithNewOrder.id !== item.id) {
        if (req.file) {
          // Delete new uploaded file if order is duplicate for another item
          fs.unlink(req.file.path, (err) => {
            if (err)
              console.error(
                "Error deleting uploaded file on duplicate order update error:",
                err
              );
          });
        }
      ...
      if (item.imageUrl) {
        ...
        if (fs.existsSync(oldImagePath)) {
          fs.unlink(oldImagePath, (err) => {
            if (err)
              console.error(
                "Error deleting old carousel image on explicit null:",
                err
              );
          });
        }
    ...
    if (req.file) {
      fs.unlink(req.file.path, (err) => {
        if (err)
          console.error(
            "Error deleting new uploaded file on update error:",
            err
          );
      });
    }
    ...
    console.log(
      "--- END Backend (CarouselController - updateCarouselItem) ---"
    );
  }
};
...
exports.deleteCarouselItem = async (req, res, next) => {
  ...
  try {
    ...
    if (item.imageUrl) {
      ...
      if (fs.existsSync(imagePath)) {
        fs.unlink(imagePath, (err) => {
          if (err)
            console.error("Error deleting carousel image on item delete:", err);
        });
      }
    }
    ...
    console.log(
      "--- END Backend (CarouselController - deleteCarouselItem) ---"
    );
  }
};
...
exports.reorderCarouselItems = async (req, res, next) => {
  ...
  try {
    ...
    if (
      ...
      console.error(
        "Reorder Error: Invalid request body. Expected an array of items with id (string) and order (number). Received:",
        items
      );
      return res
        .status(400)
        .json({
          success: false,
          message:
            "Invalid request body. Expected an array of items with id and order.",
        });
    }
    ...
    try {
      ...
      console.log(
        "Reorder: Temporarily clearing/setting orders to avoid unique constraint conflicts..."
      );
      ...
      if (existingCarouselItems.length !== itemIdsToUpdate.length) {
        ...
        console.error(
          "Reorder Error: Not all items found for reordering. Some IDs might be invalid."
        );
        return res
          .status(404)
          .json({
            success: false,
            message: "One or more carousel items not found for reordering.",
          });
      }
      ...
      console.log(
        "Reorder: Updating items with new, final orders within transaction:"
      );
      for (const item of items) {
        ...
        if (affectedRows === 0) {
          ...
          await transaction.rollback();
          console.error(
            `Reorder Error: Item with ID ${item.id} not found for final update during reordering.`
          );
          return res
            .status(404)
            .json({
              success: false,
              message: `Failed to update carousel order: Carousel item not found for ID: ${item.id} during final update.`,
            });
        }
      }
      ...
      await transaction.rollback();
      console.error(
        "Reorder Error: Transaction rolled back. Details:",
        transactionError
      );
      ...
      if (transactionError.name === "SequelizeUniqueConstraintError") {
        return res
          ...
          .json({
            success: false,
            message:
              "Erreur de contrainte unique lors de la réorganisation. Assurez-vous que tous les ordres sont distincts et qu'il n'y a pas de conflits.",
          });
      }
    ...
    console.error(
      "Reorder Error: Processing reorder request failed. Details:",
      error
    );
    next(error); // Pass error to global error handler
  } finally {
    console.log(
      "--- END Backend (CarouselController - reorderCarouselItems) ---"
    );
  }
};
