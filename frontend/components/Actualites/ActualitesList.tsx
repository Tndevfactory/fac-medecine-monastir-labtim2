...
const ActualitesList = () => {
  ...
  const ACTUALITE_CATEGORIES: ("Formation" | "Conférence" | "Laboratoire")[] = [
    "Formation",
    "Conférence",
    "Laboratoire",
  ];
  ...
  const fetchActus = useCallback(async () => {
    ...
    try {
      const response = await getAllActus(
        token,
        activeCategoryFilter,
        searchTerm
      );
      if (response.success && response.data) {
        const sortedActus = response.data.sort(
        ...
        <div className="w-full">
          {loading ? (
            ...
            <div className="lg:col-span-3 text-center text-gray-600 text-lg py-12">
              {error
                ? `Erreur: ${error}`
                : "Aucune actualité ne correspond à votre sélection."}
            </div>
          )}
