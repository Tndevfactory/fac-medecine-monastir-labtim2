...
const Actus = () => {
  ...
  if (loading) {
    return (
      <section className={`py-16 bg-white overflow-hidden min-h-[30vh] flex flex-col justify-center items-center ${inter.className}`}>
        <Loader2 className="animate-spin h-8 w-8 text-blue-600" />
        <p className="text-gray-600 text-lg mt-3">Chargement des actualités...</p>
      </section>
    );
  }
    ...
    return (
      <section className={`py-16 bg-white overflow-hidden min-h-[30vh] flex flex-col justify-center items-center ${inter.className}`}>
        ...
        <p className="text-gray-600 text-lg">
          {error ? `Erreur: ${error}` : 'Aucune actualité disponible pour le moment.'}
        </p>
      </section>
    );
  }
  ...
  return (
    <section className={`py-16 bg-white overflow-hidden min-h-[50vh] flex flex-col justify-center ${inter.className}`}>
      {toast && (
        <Toast
            ...
            >
              <div className="w-full h-48 relative overflow-hidden">
                <Image
                  ...
                  src={item.image ? `${BACKEND_BASE_URL}${item.image}` : 'https://placehold.co/600x400/E2E8F0/A0AEC0?text=Image+Non+Trouvée'}
                  alt={item.title}
                  fill
              ...
              <div className="p-4 flex flex-col justify-between h-[calc(100%-12rem)]">
                <div>
                  <p className="text-xs text-gray-500 mb-1">{formatDate(item.date)} &bull; {item.category}</p>
                  <h3 className="text-lg font-semibold text-gray-900 mb-2 line-clamp-2 group-hover:text-blue-600 transition-colors duration-200">
                    {item.title}
                  </h3>
                  <p className="text-gray-700 text-sm mb-3 line-clamp-3">{item.shortDescription}</p>
                </div>
                <button
                  className="mt-auto bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-md transition-all duration-300 text-sm self-start"
                >
                  Lire l'actualité
                </button>
              </div>
