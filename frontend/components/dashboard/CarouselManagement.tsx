...
// Lucide Icons
import {
  Image as ImageIcon, PlusCircle, Trash2, Edit3, Loader2, Info, ArrowLeft,
  ChevronDown, ChevronUp, ExternalLink, XCircle, ScrollText, FileText, Images, Save // Added Save icon
} from 'lucide-react';
...
const CarouselManagement: React.FC = () => {
  ...
  const fetchAllCarouselItems = useCallback(async (showSuccessToast = true) => { // Add parameter
    if (!token) {
    ...
    try {
      ...
      if (response.success && Array.isArray(response.data)) {
        ...
        if (showSuccessToast) { // Conditionally show toast
          setToast({ message: 'Éléments du carrousel chargés avec succès.', type: 'success' });
        }
      } else {
        setToast({ message: response.message || 'Échec du chargement des éléments du carrousel.', type: 'error' });
        setAllCarouselItems([]);
        setPreviewItems([]);
      }
    } catch (err: any) {
      console.error("Échec de la récupération des éléments du carrousel :", err);
      setToast({ message: `Erreur lors de la récupération des éléments du carrousel : ${err.message || 'Veuillez réessayer plus tard.'}`, type: 'error' });
      setAllCarouselItems([]);
      setPreviewItems([]);
    } finally {
      setLoadingItems(false);
  ...
  }, [authLoading, isAuthenticated, userRole, token, router, fetchAllCarouselItems]);
  ...
  const handleEditItem = useCallback(async (itemId: string) => {
    ...
    try {
      ...
      if (!itemToEdit) {
        setToast({ message: 'Élément de carrousel introuvable pour édition.', type: 'error' });
        return;
      }
      ...
      const updatedPreviewItems = allCarouselItems.map(item =>
        item.id === itemToEdit.id ? { ...itemToEdit, imageUrl: itemToEdit.imageUrl } : item // Use actual fetched image URL
      ).sort((a, b) => a.order - b.order);
      setPreviewItems(updatedPreviewItems);
      setToast({ message: 'Élément du carrousel chargé pour modification.', type: 'info' });

    } catch (err: any) {
      console.error("Échec de la récupération de l'élément pour édition :", err);
      setToast({ message: `Erreur lors du chargement de l'élément pour édition : ${err.message || 'Veuillez réessayer.'}`, type: 'error' });
    } finally {
      setLoadingItems(false);
    }
  ...
  useEffect(() => {
    if (currentView === 'add' || currentView === 'edit') {
      ...
      const currentPreviewItem: CarouselItem = {
        ...
        order: Number(order) || (allCarouselItems.length > 0 ? Math.max(...allCarouselItems.map(i => i.order)) + 1 : 1),
        link: link || '',
      };
  ...
  }, [title, description, order, link, imagePreviewUrl, editingItem, allCarouselItems, currentView]);
  ...
  const handleSaveItem = async () => {
    ...
    if (order === '' || isNaN(Number(order))) {
      setError('L\'ordre est un champ obligatoire et doit être un nombre valide.');
    ...
    console.log('handleSaveItem: Item data being sent:', Object.fromEntries(formData.entries())); // Log FormData content

    try {
      if (editingItem?.id) {
        console.log(`handleSaveItem: Calling apiUpdateCarouselItem for ID: ${editingItem.id}`);
        await apiUpdateCarouselItem(editingItem.id, formData, token);
        setToast({ message: 'Élément du carrousel mis à jour avec succès !', type: 'success' }); // Use setToast
      } else {
        console.log('handleSaveItem: Calling apiCreateCarouselItem for new item.');
        await apiCreateCarouselItem(formData, token);
        setToast({ message: 'Élément du carrousel créé avec succès !', type: 'success' }); // Use setToast
      }
      ...
      console.error("Échec de l'enregistrement de l'élément du carrousel :", err);
      setToast({ message: `Échec de l'enregistrement : ${err.message || 'Veuillez réessayer.'}`, type: 'error' }); // Use setToast for error
    } finally {
  ...
  const handleDeleteItem = useCallback(async () => {
    if (!confirmDeleteItemId || !token) {
      setToast({ message: 'ID de l\'élément ou jeton manquant.', type: 'error' }); // Use setToast
      return;
    }
    setIsSubmitting(true);
    ...
    try {
      ...
      setToast({ message: 'Élément du carrousel supprimé avec succès !', type: 'success' }); // Use setToast
      ...
      setToast({ message: `Échec de la suppression : ${err.message || 'Veuillez réessayer.'}`, type: 'error' }); // Use setToast for error
    } finally {
      setIsSubmitting(false);
    }
  ...
  const handleDragStart = (e: React.DragEvent<HTMLTableRowElement>, item: CarouselItem) => {
    setDraggedItem(item);
    e.dataTransfer.effectAllowed = 'move';
    e.dataTransfer.setData('text/plain', item.id!); // Store the ID of the dragged item
  };

  const handleDragOver = (e: React.DragEvent<HTMLTableRowElement>, targetItem: CarouselItem) => {
  ...
  const handleDrop = (e: React.DragEvent<HTMLTableRowElement>, targetItem: CarouselItem) => {
    e.preventDefault();
    // Optional: Remove visual feedback
    e.currentTarget.classList.remove('bg-blue-50'); // Remove the class
  ...
  const handleSaveOrder = async () => {
    ...
    try {
      ...
      if (itemsToReorder.length === 0) {
        setToast({ message: 'Aucun élément valide à réorganiser trouvé. Assurez-vous que tous les éléments sont enregistrés.', type: 'warning' });
        setIsSubmitting(false);
        return;
      }

      console.log('Frontend: Sending items to backend for reorder:', itemsToReorder); // Log what's being sent

      // Call the new batch update API function
      await updateCarouselItemOrder(itemsToReorder, token);

      setToast({ message: 'L\'ordre du carrousel a été sauvegardé avec succès !', type: 'success' });
      ...
      setToast({ message: `Échec de la sauvegarde de l'ordre : ${err.message || 'Veuillez réessayer.'}`, type: 'error' });
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  if (authLoading || loadingItems) {
    return (
      <div className={`flex justify-center items-center h-screen bg-gray-50 ${inter.className}`}>
        ...
        <span className="ml-3 text-lg text-gray-700">Chargement des éléments du carrousel...</span>
      </div>
    );
  }
    ...
    <div className={`min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-8 ${inter.className}`}>
      <header className=" text-center">
        <div className="flex items-center space-x-4 bg-gradient-to-r from-blue-600 to-purple-700 text-white p-6 rounded-xl shadow-lg mb-8">
          ...
          <h1 className="text-4xl font-extrabold tracking-tight">Gestion de la Carousel</h1>
          <p className="text-xl text-white">Gérez les éléments de la carousel</p>
        </div>
      </header>
      ...
      {currentView === 'list' && (
        <div className="bg-white p-8 rounded-xl shadow-md border border-gray-100">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-semibold text-gray-800">Éléments du Carrousel</h2>
            <button
          ...
          {allCarouselItems.length === 0 && !loadingItems && (
            <p className="text-center text-gray-500 py-8">Aucun élément de carrousel trouvé. Cliquez sur "Ajouter un nouvel élément" pour en créer un.</p>
          )}
          ...
          {allCarouselItems.length > 0 && (
            <>
              <p className="text-sm text-gray-600 mb-4 flex items-center">
                ...
                Vous pouvez réorganiser les éléments du carrousel en les faisant glisser et déposer (drag-and-drop) directement dans le tableau. N'oubliez pas de sauvegarder l'ordre après modification.
              </p>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {allCarouselItems.map((item) => (
                      <tr
                        ...
                        className={`hover:bg-gray-50 ${draggedItem?.id === item.id ? 'opacity-50 border-2 border-dashed border-blue-500' : ''}`}
                      >
                          ...
                          {item.imageUrl ? (
                            <Image
                              ...
                              onError={(e) => {
                                ...
                                target.src = 'https://placehold.co/80x45/cccccc/333333?text=N/A'; // Fallback
                              }}
                            />
                          ) : (
                        ...
                        <td className="px-6 py-4 text-sm font-medium text-gray-900 max-w-xs truncate" title={item.title || ''}>{item.title || 'N/A'}</td>
                        <td className="px-6 py-4 text-sm text-gray-500 max-w-xs truncate" title={item.description || ''}>{item.description || 'N/A'}</td>
                        <td className="px-6 py-4 text-sm text-blue-600 max-w-xs truncate">
                          {item.link ? (
                            <a href={item.link} target="_blank" rel="noopener noreferrer" className="hover:underline flex items-center">
      ...
      {(currentView === 'add' || currentView === 'edit') && (
        <div className="bg-white p-8 rounded-xl shadow-md border border-gray-100 relative mt-8">
          ...
          <h2 className="text-3xl font-bold text-gray-900 mb-8 text-center mt-4">
            {editingItem ? 'Modifier l\'élément du carrousel' : 'Ajouter un nouvel élément de carrousel'}
          </h2>

          {/* Live Carousel Preview */}
          <div className="mb-10 p-4 border border-blue-200 rounded-lg bg-blue-50">
            <h3 className="text-xl font-semibold text-blue-800 mb-4 text-center">Aperçu du Carrousel</h3>
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleSaveItem(); }} className="space-y-6">
            {/* Image Upload */}
            <div>
              <label htmlFor="imageFile" className="block text-sm font-medium text-gray-700 mb-2 flex items-center">
                <ImageIcon className="w-4 h-4 mr-1 text-gray-500" /> Image {currentView === 'add' && <span className="text-red-500">*</span>}
              </label>
              {imagePreviewUrl && (
                <div className="mb-4 flex items-center space-x-4">
                  <Image src={imagePreviewUrl} alt="Aperçu de l'image" width={150} height={84} className="rounded-md object-cover border border-gray-300" />
                  <button
                    type="button"
                    onClick={() => {
                      setImageFile(null);
            ...
            <div>
              <label htmlFor="order" className="block text-sm font-medium text-gray-700 mb-2 flex items-center">
                <ImageIcon className="w-4 h-4 mr-1 text-gray-500" /> Ordre d'affichage <span className="text-red-500">*</span>
              </label>
              <input
                type="number"
                id="order"
            ...
            <div>
              <label htmlFor="description" className="block text-sm font-medium text-gray-700 mb-2 flex items-center">
                <ScrollText className="w-4 h-4 mr-1 text-gray-500" /> Description
              </label>
              <textarea
                id="description"
                value={description}
            ...
            {/* Link */}
            <div>
              <label htmlFor="link" className="block text-sm font-medium text-gray-700 mb-2 flex items-center">
                <ExternalLink className="w-4 h-4 mr-1 text-gray-500" /> Lien (URL)
              </label>
              <input
          ...
          <div className="relative p-8 bg-white w-full max-w-md m-auto flex-col flex rounded-lg shadow-lg">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-500 mt-2">
                Êtes-vous sûr de vouloir supprimer cet élément du carrousel ? Cette action est irréversible.
              </p>
            </div>
