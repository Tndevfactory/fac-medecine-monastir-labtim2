...
// Lucide Icons
import {
  Newspaper, PlusCircle, Trash2, Edit3, Eye, Loader2, Info, ArrowLeft,
  Calendar as CalendarIcon, Image as ImageIcon, Tag, Layout, Text, Heading, List,
  Bold, Palette, ChevronDown, ChevronUp, GripVertical, Search
} from 'lucide-react';
...
const ActuManagement: React.FC = () => {
  ...
  const [confirmDeleteActuId, setConfirmDeleteActuId] = useState<string | null>(null);
  ...
  const [category, setCategory] = useState<'Conférence' | 'Formation' | 'Laboratoire'>('Conférence'); 
  ...
  const fetchActus = useCallback(async (showSuccessToast = true) => {
    if (!token) {
      setErrorModal({
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (showSuccessToast && response.data.length > 0) {
          setToast({ message: 'Actualités chargées avec succès.', type: 'success' });
        }
      } else {
        console.error("Échec de la récupération des actualités :", response.message);
        setErrorModal({
  ...
  useEffect(() => {
    ...
    const newFilteredActus = actus.filter(actu =>
  ...
  const handleEditActu = useCallback(async (actuId: string) => {
    if (!token) {
      setErrorModal({
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (actuToEdit.image) {
          const imageUrl = actuToEdit.image.startsWith('/uploads') 
            ? `${BACKEND_BASE_URL}${actuToEdit.image}`
            : actuToEdit.image;
        ...
        setIsEditing(true);
        setCurrentView('edit');
        setToast({ message: 'Actualité chargée pour modification.', type: 'info' });
      } else {
        console.error("Échec de la récupération de l'actualité pour édition :", response.message);
        setErrorModal({
          title: 'Actualité non trouvée',
          briefDescription: response.message || 'Impossible de trouver l\'actualité spécifiée.',
          detailedError: response.message,
        });
      }
    } catch (err: any) {
      console.error("Échec de la récupération de l'actualité pour édition (catch) :", err);
      setErrorModal({
        title: 'Erreur réseau ou serveur',
        briefDescription: 'Impossible de communiquer avec le serveur ou erreur inattendue.',
  ...
  const handleViewActu = useCallback(async (actuId: string) => {
    if (!token) {
      setErrorModal({
        title: 'Jeton d\'authentification introuvable.',
        briefDescription: 'Veuillez vous connecter.',
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setCurrentView('view');
        setToast({ message: 'Actualité chargée pour visualisation.', type: 'info' });
      } else {
        console.error("Échec de la récupération de l'actualité pour visualisation :", response.message);
        setErrorModal({
          title: 'Actualité non trouvée',
          briefDescription: response.message || 'Impossible de trouver l\'actualité spécifiée.',
          detailedError: response.message,
      ...
      console.error("Échec de la récupération de l'actualité pour visualisation (catch) :", err);
      setErrorModal({
        title: 'Erreur réseau ou serveur',
        briefDescription: 'Impossible de communiquer avec le serveur ou erreur inattendue.',
        detailedError: err.message || 'Veuillez réessayer.',
  ...
  const handleConfirmSave = async () => {
    ...
    if (!title || !date || !shortDescription || !category) {
      setToast({ message: 'Veuillez remplir tous les champs obligatoires (Titre, Date, Courte Description, Catégorie).', type: 'error' });
      return;
    }
    ...
    if (userId) {
      ...
      setToast({ message: 'L\'ID utilisateur est manquant. Impossible de créer l\'actualité sans utilisateur lié.', type: 'error' });
    ...
    try {
      ...
      if (isEditing && currentActu?.id) {
        ...
        if (response.success) {
          setToast({ message: 'Actualité mise à jour avec succès !', type: 'success' });
        } else {
          throw new Error(response.message || 'Échec de la mise à jour de l\'actualité.');
        }
      } else {
        response = await apiCreateActu(formData, token);
        if (response.success) {
          setToast({ message: 'Actualité créée avec succès !', type: 'success' });
        } else {
          throw new Error(response.message || 'Échec de la création de l\'actualité.');
        }
      }
      resetForm();
      setCurrentView('list');
      ...
      setToast({ message: `Échec de l'enregistrement de l'actualité : ${err.message || 'Veuillez réessayer.'}`, type: 'error' });
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleDeleteActu = useCallback(async () => {
    ...
    try {
      ...
      if (response.success) {
        setToast({ message: 'Actualité supprimée avec succès !', type: 'success' });
      } else {
        throw new Error(response.message || 'Échec de la suppression de l\'actualité.');
      }
      setConfirmDeleteActuId(null); 
      await fetchActus(false);
    } catch (err: any) {
      console.error("Échec de la suppression de l'actualité :", err);
      setToast({ message: `Échec de la suppression de l'actualité : ${err.message || 'Veuillez réessayer.'}`, type: 'error' });
    } finally {
      setIsSubmitting(false);
    }
  ...
  const previewActu: Actu = useMemo(() => ({
    ...
    image: imagePreviewUrl || 'https://placehold.co/600x400/E0E0E0/4A4A4A?text=Image+de+pr%C3%A9visualisation', // Placeholder image
    date: date || getTodaysDate(),
    category: category || 'Conférence', // Use current category state
    shortDescription: shortDescription || 'Ceci est un aperçu de votre actualité.',
  ...
  }), [title, imagePreviewUrl, date, category, shortDescription, fullContent, userId]);
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-8  ${inter.className}`}>
      <div className="flex items-center space-x-4 bg-gradient-to-r from-blue-600 to-purple-700 text-white p-6 rounded-xl shadow-lg mb-8">
          ...
          <h1 className="text-4xl font-extrabold tracking-tight">Gestion des Actualités</h1>
        <p className="text-xl text-white  ">Gérez, créez et publiez les actualités de votre plateforme.</p>
        </div>
      ...
      {currentView === 'list' && (
        <div className="bg-white p-8 rounded-xl shadow-md border border-gray-100">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-semibold text-gray-800">Toutes les actualités</h2>
          ...
          {displayedActus.length === 0 && !loading && (
            <p className="text-center text-gray-500 py-8">Aucune actualité trouvée. Cliquez sur "Ajouter une nouvelle actualité" pour en créer une.</p>
          )}
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Description courte</th>
                  ...
                  {displayedActus.map((actu) => (
                    <tr key={actu.id} className="hover:bg-gray-50">
                          ...
                          {actu.image && (
                            <div className="flex-shrink-0 h-10 w-10">
                              <Image
                                ...
                                src={actu.image.startsWith('/uploads') ? `${BACKEND_BASE_URL}${actu.image}` : actu.image}
                                alt={actu.title}
                                width={40}
                                height={40}
                                onError={(e) => {
                                  (e.target as HTMLImageElement).onerror = null;
                                  (e.target as HTMLImageElement).src = 'https://placehold.co/40x40/E0E0E0/4A4A4A?text=N/A';
                                }}
                              />
                            </div>
                          )}
                          <div className={`${actu.image ? 'ml-4' : ''}`}>
                            <div className="text-sm font-medium text-gray-900">{actu.title}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                          actu.category === 'Conférence' ? 'bg-blue-100 text-blue-800' :
                          actu.category === 'Formation' ? 'bg-purple-100 text-purple-800' :
                          actu.category === 'Laboratoire' ? 'bg-green-100 text-green-800' :
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {new Date(actu.date).toLocaleDateString('fr-FR', { year: 'numeric', month: 'short', day: 'numeric' })}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {actu.creatorName || 'N/A'}
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-500 truncate max-w-xs" title={actu.shortDescription}>
            ...
            >
              <div className="hidden sm:block">
                <p className="text-sm text-gray-700">
                  ...
                  <span className="font-medium">{Math.min(currentPage * ITEMS_PER_PAGE, filteredActus.length)}</span> sur{' '}
                  <span className="font-medium">{filteredActus.length}</span> résultats
                </p>
              </div>
      ...
      {(currentView === 'add' || currentView === 'edit') && (
        <div className="bg-white p-8 rounded-xl shadow-md border border-gray-100 relative">
          ...
          <h2 className="text-3xl font-bold text-gray-900 mb-8 text-center mt-4">
            {isEditing ? 'Modifier l\'actualité' : 'Créer une nouvelle actualité'}
          </h2>
            ...
            <div>
              <div className="mb-6">
                <label htmlFor="title" className="block text-sm font-medium text-gray-700 mb-2">
                  <Heading className="inline-block w-4 h-4 mr-1 text-gray-500" /> Titre <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  id="title"
              ...
              <div className="mb-6">
                <label htmlFor="category" className="block text-sm font-medium text-gray-700 mb-2">
                  <Tag className="inline-block w-4 h-4 mr-1 text-gray-500" /> Catégorie <span className="text-red-500">*</span>
                </label>
                <select
                  id="category"
                  value={category}
                  // Removed 'SHC' from the type assertion
                  onChange={(e) => setCategory(e.target.value as 'Conférence' | 'Formation' | 'Laboratoire')} 
                  className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                  required
                >
              ...
              <div className="mb-6">
                <label htmlFor="date" className="block text-sm font-medium text-gray-700 mb-2">
                  <CalendarIcon className="inline-block w-4 h-4 mr-1 text-gray-500" /> Date <span className="text-red-500">*</span>
                </label>
                <input
                  type="date"
                  id="date"
              ...
              <div className="mb-6">
                <label htmlFor="shortDescription" className="block text-sm font-medium text-gray-700 mb-2">
                  <List className="inline-block w-4 h-4 mr-1 text-gray-500" /> Description courte <span className="text-red-500">*</span>
                </label>
                <textarea
                  id="shortDescription"
                  ...
                  required
                ></textarea>
                <p className="text-right text-xs text-gray-500">{shortDescription.length}/500</p>
              </div>
            </div>
            ...
            <div>
              <div className="mb-6">
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  <ImageIcon className="inline-block w-4 h-4 mr-1 text-gray-500" /> Image de l'actualité
                </label>
          ...
          <div className="mb-8">
            <label htmlFor="fullContent" className="block text-sm font-medium text-gray-700 mb-2">
              <Text className="inline-block w-4 h-4 mr-1 text-gray-500" /> Contenu complet
            </label>
            <ReactQuill
              theme="snow"
              value={fullContent}
          ...
          <div className="mt-10 pt-6 border-t border-gray-200">
            <h3 className="text-xl font-bold text-gray-900 mb-4 flex items-center">
              <Eye className="inline-block w-5 h-5 mr-2 text-indigo-600" /> Aperçu en direct
            </h3>
            <p className="text-gray-600 mb-6">Voyez à quoi ressemblera votre actualité sur la page publique :</p>
