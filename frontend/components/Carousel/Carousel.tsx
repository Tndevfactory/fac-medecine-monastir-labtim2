...
const Carousel: React.FC<CarouselProps> = ({ slides }) => {
  ...
  if (totalSlides === 0) {
    return (
      <div className={`relative w-full h-[33vh] flex items-center justify-center bg-gray-200 text-gray-600 rounded-lg shadow-xl ${inter.className}`}>
  ...
  return (
    <section className={`relative w-full overflow-hidden my-8 rounded-lg shadow-xl ${inter.className}`}>
      {/* Slides Container */}
      <div
      ...
      >
        {slides.map((slide) => (
          ...
          <div key={slide.id} className="w-full flex-shrink-0 relative h-[33vh]">
            {/* Image */}
            <Image
              src={slide.imageUrl}
              ...
              onError={(e) => {
                ...
                target.src = 'https://placehold.co/1260x750/cccccc/333333?text=Image+introuvable'; // Fallback
        ...
        {slides.map((_, index) => (
          <button
            ...
            className={`h-2.5 w-2.5 rounded-full bg-white transition-all duration-300 ${
              currentSlide === index ? 'w-6 bg-blue-500' : 'bg-opacity-50 hover:bg-opacity-75'
            }`}
