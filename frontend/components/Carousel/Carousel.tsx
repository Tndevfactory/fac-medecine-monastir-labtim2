...
const Carousel: React.FC = () => {
  console.log("API_BASE_URL in Carousel component:", API_BASE_URL); // Debug log to verify the base URL

  useEffect(() => {
    console.log("Fetching carousel data from:", `${API_BASE_URL}/carousel`);
  }, []);
  ...
  useEffect(() => {
    ...
    const fetchCarouselData = async () => {
      try {
        ...
        console.log("response fetchCarouselData in carousel", data);
  ...
  if (totalSlides === 0) {
    return (
      <div
        className={`relative w-full h-[33vh] flex items-center justify-center bg-gray-200 text-gray-600 rounded-lg shadow-xl ${inter.className}`}
      >
  ...
  return (
    <section
      className={`relative w-full overflow-hidden my-8 rounded-lg shadow-xl ${inter.className}`}
    >
      {/* Slides Container */}
      <div
